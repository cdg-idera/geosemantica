# Capítulo: **Entrenamientos Supervisados con Embeddings**
...
2) **Búsqueda: Hornos de ladrillo con filtros *post* “fail-open”.**  
...
## ¿Qué aprenderás (learning outcomes)?
...
## Buenas prácticas que atraviesan los ejemplos

- **Normalización y saneo:** controlar *no-data* con `unmask(0)` antes de reducir; verificar coberturas.  
- **Umbrales interpretables:** exponer **≥ 0.98/0.99/≈ 1.0** para interpretabilidad; complementar con **percentil adaptativo** en escenarios complejos.  
...
1. **Búsqueda: Agua por similitud** — construye el concepto de “prototipo de clase” y muestra su lectura como isócronas de similitud.  
2. **Búsqueda: Ladrilleras + *fail-open*** — enseña a **endurecer** hipótesis con filtros espectrales/radar sin perder hallazgos.  
3. **Clasificación de cultivos** — cierra integrando embeddings con fenología y máscaras, más evaluación y UI.
...
$$
\textbf{Entradas: }(\text{ROI},\ \text{muestras(agua)}) 
\xrightarrow[]{\text{Parámetros}}
\text{Embedding anual } \mathbf{X} 
\xrightarrow[\text{occurrence}\,\ge\,\text{JRC\_OCC\_MIN}]{\text{Máscara JRC (opcional)}}
\mathbf{X}_{\text{mask}} 
\xrightarrow[\text{promedio por polígono}]{\text{Vectores } \{\mathbf{s}_i\}}
\max_i\,\cos(\mathbf{s}_i,\mathbf{x})\ (0\!\le\!\cdot\!\le\!1)
\xrightarrow[\ge 0.98,\ \ge 0.99,\ \approx 1.0]{\text{Vectorizar + área mín.}}
\text{Polígonos de agua}
\xrightarrow[]{\text{Exportación / UI}}
$$
...
$$
\textbf{Entradas: }(\text{ROI},\ \text{muestras}) 
\xrightarrow[\text{año, escala, umbrales}]{\text{Parámetros}}
\text{Embedding anual } \mathbf{X} 
\xrightarrow[\text{promedio por polígono}]{\text{Vectores de referencia } \{\mathbf{s}_i\}}
\max_i\,\cos(\mathbf{s}_i,\mathbf{x})\ (0\!\le\!\cdot\!\le\!1)
\xrightarrow[\text{NDVI/NDBI/BSI,\ S1 VV/VH,\ agua}]{\text{Filtros POST (fail-open)}}
\text{Similitud enmascarada}
\xrightarrow[\ge 0.98,\ \ge 0.99,\ \approx 1.0]{\text{Vectorizar + área mín.}}
\text{Polígonos candidatos}
\xrightarrow[\text{P99 (fallback 0.95)}]{\text{Umbral adaptativo}}
\text{Capa final}
\xrightarrow[]{\text{Exportación / UI}}
$$
